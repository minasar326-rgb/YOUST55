...
async function generateWordReport(student, records, filename) {
  try {
    ...
    const htmlContent = `<h2>تقرير ${student.name}</h2><table border="1"><tr><th>التاريخ</th><th>الوقت</th><th>اليوم</th><th>الجزء</th><th>الحالة</th></tr>${records.map(r => `<tr><td>${r.date}</td><td>${r.time}</td><td>${r.dayName}</td><td>${getPartName(r.part)}</td><td>${r.status === 'present' ? 'حاضر' : 'غائب'}</td></tr>`).join('')}</table>`;
